  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-navy-900 tracking-tight">Blog / Updates</h2>
          <p className="text-sm text-navy-500 mt-1">{posts.length} posts · shown on homepage feed</p>
        </div>
        <button
      ...
      <div className="space-y-3">
        ...
        {posts.map(post => (
          <div
            key={post.id}
            className="bg-white border border-navy-100 rounded-2xl p-5 flex items-start gap-4 shadow-sm"
          >
            {post.image_url && (
              <img
                src={post.image_url}
                alt={post.title}
                className="w-16 h-16 rounded-xl object-cover shrink-0 border border-navy-100"
              />
            )}
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <h3 className="font-semibold text-navy-900 truncate">{post.title}</h3>
                <span className="text-xs text-navy-400">{timeAgo(post.created_at)}</span>
              </div>
              {post.content && (
                <p className="text-sm text-navy-500 line-clamp-2">{post.content}</p>
              )}
              {post.image_url && (
                <a
                  href={post.image_url}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl">
            ...
            <div className="flex items-center justify-between p-6 border-b border-navy-100">
              <h3 className="font-bold text-navy-900 text-lg">New Announcement</h3>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 border border-navy-200 rounded-xl text-sm text-navy-900 focus:outline-none focus:ring-2 focus:ring-gold-400 focus:border-transparent"
                  required
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full px-4 py-2.5 border border-navy-200 rounded-xl text-sm text-navy-900 resize-none focus:outline-none focus:ring-2 focus:ring-gold-400 focus:border-transparent"
                />
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-navy-700 mb-1.5 flex items-center gap-1.5">
                  ...
                  Image URL <span className="text-navy-400 font-normal">(optional — timetable, etc.)</span>
                </label>
                <input
                  type="url"
                  value={imageUrl}
                  ...
                  className="w-full px-4 py-2.5 border border-navy-200 rounded-xl text-sm text-navy-900 focus:outline-none focus:ring-2 focus:ring-gold-400 focus:border-transparent"
                />
                <p className="text-xs text-navy-400 mt-1">Paste a Google Drive or direct image link. A download button will appear automatically.</p>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setShowModal(false)}
                  className="flex-1 px-4 py-2.5 rounded-xl border border-navy-200 text-sm font-medium text-navy-600 hover:bg-navy-50 transition-colors"
                >
                  Cancel
                </button>
                <button
