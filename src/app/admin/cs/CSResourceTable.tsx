        ...
        <div className="relative flex-1">
          ...
          {search && (
            <button onClick={() => setSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2">
              <X className="w-4 h-4 text-[var(--text-muted)] hover:text-[var(--text-muted)]" />
            </button>
          )}
        </div>
        <button
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-[var(--bg-card)] rounded-2xl w-full max-w-lg shadow-xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1">Source URL *</label>
            <input
              required
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1">Topic (optional)</label>
            <input
              value={form.topic}
