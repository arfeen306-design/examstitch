  ...
  return (
    ...
    >
      <div className="bg-white/[0.06] backdrop-blur-xl border border-white/[0.1] rounded-2xl p-6">
        ...
        <p className="text-sm text-white/50 mb-4">Be the first to know when new past papers and video solutions are uploaded.</p>
        <form onSubmit={handleSubmit} className="flex gap-2">
          <input
            type="email"
            value={email}
            ...
            placeholder="your@email.com"
            required
            className="flex-1 px-4 py-2.5 bg-white/[0.06] border border-white/[0.1] rounded-xl text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-gold-500/50 focus:ring-1 focus:ring-gold-500/20 transition-all"
          />
          <button
            type="submit"
            disabled={status === 'loading'}
